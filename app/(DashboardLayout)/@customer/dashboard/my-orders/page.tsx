...
const STATUS_CONFIG = {
  PENDING: { color: "text-amber-500 bg-amber-500/10 border-amber-500/20", icon: Clock, step: 0 },
  CONFIRMED: { color: "text-blue-500 bg-blue-500/10 border-blue-500/20", icon: CheckCircle, step: 1 },
  COOKING: { color: "text-orange-500 bg-orange-500/10 border-orange-500/20", icon: ChefHat, step: 2 },
  DELIVERED: { color: "text-emerald-500 bg-emerald-500/10 border-emerald-500/20", icon: Truck, step: 3 },
  CANCELLED: { color: "text-rose-500 bg-rose-500/10 border-rose-500/20", icon: Ban, step: -1 },
} as const;
    ...
    <div className="p-6 md:p-12 max-w-[1400px] mx-auto space-y-10 animate-in fade-in duration-700">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8">
        <div className="space-y-4">
          <h1 className="text-7xl font-black text-foreground tracking-tighter uppercase italic leading-[0.8]">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
            ...
            {(["ACTIVE", "HISTORY", "CANCELLED"] as const).map((filter) => (
              <button
                ...
                className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                  viewFilter === filter ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                {filter}
              </button>
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full sm:w-64 h-12 bg-card border border-border rounded-xl pl-12 pr-4 text-[10px] font-black uppercase tracking-widest outline-none focus:border-orange-500 transition-all shadow-sm text-foreground" 
            />
          </div>
        </div>
      </div>

      <div className="bg-card border border-border rounded-[3rem] overflow-hidden shadow-2xl shadow-slate-500/10">
        <Table>
          <TableHeader className="bg-muted/50">
            <TableRow className="hover:bg-transparent border-border">
              <TableHead className="p-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Reference</TableHead>
              <TableHead className="p-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Meal Manifest</TableHead>
              <TableHead className="p-8 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Status</TableHead>
              <TableHead className="p-8 text-right text-[10px] font-black uppercase tracking-widest text-muted-foreground">Operations</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filteredOrders.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} className="p-20 text-center">
                  ...
                  <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">No entries found for {viewFilter}</p>
                </TableCell>
              </TableRow>
            ) : filteredOrders.map((order) => (
              <TableRow key={order.id} className="hover:bg-muted/30 border-border transition-colors">
                <TableCell className="p-8">
                  <div className="space-y-1">
                    <p className="font-black text-foreground text-sm tracking-tighter uppercase">#{order.orderNumber}</p>
                ...
                <TableCell className="p-8">
                  <div className="flex flex-wrap gap-2">
                    {order.items.map((item) => (
                      <div key={item.id} className="group relative flex items-center bg-muted px-3 py-2 rounded-xl border border-border hover:border-orange-200 transition-all">
                        ...
                        <span className="text-[9px] font-black text-foreground uppercase">{item.quantity}x {item.meal.name}</span>
                      </div>
                    ))}
                  </div>
                </TableCell>
                  ...
                  <div className="flex justify-end gap-3">
                    ...
                    {order.status === "PENDING" && (
                      <button 
                        ...
                        className="h-10 px-6 bg-background border-2 border-border text-rose-500 rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 hover:bg-rose-500 hover:text-white transition-all active:scale-95 disabled:opacity-50"
                      >
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <button 
          ...
          className="h-10 px-6 bg-foreground text-background rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 hover:bg-orange-600 hover:text-white transition-all active:scale-95 shadow-xl shadow-black/5"
        >
          <Navigation size={12} /> Trace
        </button>
      </DialogTrigger>
      <DialogContent className="rounded-[3rem] border border-border shadow-2xl p-0 overflow-hidden max-w-xl bg-card">
        <DialogHeader className="sr-only">
          <DialogTitle>Order Tracking - {order.orderNumber}</DialogTitle>
        </DialogHeader>
        
        <div className={`p-8 text-white ${order.status === 'CANCELLED' ? 'bg-rose-500' : 'bg-foreground dark:bg-muted'}`}>
            ...
            <div className="relative flex items-center justify-between w-full">
              ...
              {[
                ...
                <div key={s.step} className="relative z-10 flex flex-col items-center gap-3">
                  <div className={`h-10 w-10 rounded-full flex items-center justify-center border-4 border-card transition-all duration-500 shadow-xl ${
                    currentStep >= s.step ? "bg-orange-500 text-white scale-110" : "bg-muted text-muted-foreground"
                  }`}>
                    <s.icon size={16} />
                  </div>
                  <p className={`text-[8px] font-black uppercase tracking-widest ${currentStep >= s.step ? "text-foreground" : "text-muted-foreground"}`}>
                    {s.label}
                  </p>
                </div>
              ))}
