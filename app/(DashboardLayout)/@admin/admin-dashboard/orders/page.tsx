...
export default async function OrderHistoryPage() {
  const orders = await getAllOrdersAction({ limit: 15 });

  return (
    <div className="p-4 md:p-8 space-y-8 animate-in fade-in duration-500">
      <div className="max-w-[1400px] mx-auto border-b border-border pb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-foreground tracking-tighter uppercase italic">
            Order <span className="text-orange-500">Flux.</span>
          </h1>
          <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-[0.4em] mt-1">
            System Operations / Worldwide Registry
          </p>
        </div>
        
        <div className="flex gap-4">
          <div className="bg-card border border-border px-5 py-3 rounded-2xl flex flex-col justify-center">
