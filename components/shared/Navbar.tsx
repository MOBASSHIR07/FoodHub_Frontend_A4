...
import { LogOut, Search, ShoppingBag, Menu, UtensilsCrossed } from "lucide-react";
...
export default function Navbar() {
  ...
  if (!mounted) {
    return (
      <header className="sticky top-0 z-50 w-full border-b border-border bg-background/80 backdrop-blur-md h-20">
        <div className="container mx-auto px-6 flex items-center justify-between h-full">
          <div className="flex items-center gap-2">
            <div className="h-10 w-10 bg-orange-600 rounded-xl" />
            <span className="text-xl font-black text-foreground">Food<span className="text-orange-600">Hub</span></span>
          </div>
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={cn(
                ...
                pathname === link.href ? "text-orange-600 underline underline-offset-8 decoration-2" : "text-muted-foreground"
              )}
            >
              {link.name}
            </Link>
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          {session ? (
            <div className="flex items-center gap-3">
               <span className="hidden xl:block text-[10px] font-black uppercase text-muted-foreground tracking-widest">
                 Hi, {session.user.name.split(" ")[0]}
               </span>
               <Button 
                onClick={handleSignOut}
                variant="outline"
                className="hidden md:flex rounded-full px-6 font-bold border-orange-600 text-orange-600 hover:bg-orange-500/10 transition-all active:scale-95"
              >
            ...
            <Link href="/sign-in">
              <Button className="hidden md:flex rounded-full bg-orange-600 px-6 font-bold text-white hover:bg-orange-700 transition-all active:scale-95 shadow-md shadow-orange-500/20">
                Sign In
              </Button>
            </Link>
          )}
              ...
              <SheetContent side="right" className="w-75">
                ...
                <nav className="flex flex-col gap-6 mt-10">
                    ...
                    <Link
                      key={link.name}
                      href={link.href}
                      className={cn("text-lg font-bold transition-colors", pathname === link.href ? "text-orange-600" : "text-foreground hover:text-orange-600")}
                    >
                      {link.name}
                    </Link>
                  ))}
